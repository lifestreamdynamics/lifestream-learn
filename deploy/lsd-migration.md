...
projects on `REDACTED-VPS-HOST`). This doc covers what changed, the
...
```bash
make install
export LSD_VAULT_HOST=REDACTED-VPS-HOST   # if not already in your shell rc
lsd doctor
```

`lsd doctor` should report `✓ lsd-vault agent ...`. If it doesn't, run
`lsd vault init --host=REDACTED-VPS-HOST` once.
...
scp infra/nginx/secure_link.conf.inc \
    root@REDACTED-VPS-HOST:/etc/nginx/snippets/learn-api-secure_link.conf.inc
ssh root@REDACTED-VPS-HOST 'chmod 0644 /etc/nginx/snippets/learn-api-secure_link.conf.inc'
```
...
scp root@REDACTED-VPS-HOST:/etc/learn-api/.env /run/user/$UID/lsd-import/learn-api.env
...
curl -sI https://learn-api.REDACTED-BRAND-DOMAIN/health
ssh root@REDACTED-VPS-HOST 'sudo -u learn-api pm2 list'
```
...
- **`learn-tusd` and `learn-seaweedfs` are NOT touched by `lsd deploy
  ...
  manually: `ssh root@REDACTED-VPS-HOST sudo -u learn-api pm2 reload learn-tusd`.
