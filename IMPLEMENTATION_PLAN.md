...
- **Revenue model:** subscriptions / per-course fees for the hosted service at `learn.REDACTED-BRAND-DOMAIN`. Content (videos, cues) and user data are served only through authenticated APIs and are not part of the open-source distribution.
...
- `learn.REDACTED-BRAND-DOMAIN` — API only (+ minimal landing page with terms/privacy/app-store links)
...
```
┌──────────────────────────┐        ┌────────────────────────────────────┐
│  Flutter App (Android)   │        │  REDACTED-VPS-HOST (VPS)           │
│  ─────────────────────   │        │  ──────────────────────────────    │
│  • TikTok-style feed     │ HTTPS  │  Nginx (TLS, routing, secure_link) │
│  • Interactive overlays  │◀──────▶│    ├─ learn.REDACTED-BRAND-DOMAIN │
...
| Domain | `learn.REDACTED-BRAND-DOMAIN` |
...
**Tasks:**
1. DNS: add `learn.REDACTED-BRAND-DOMAIN` A record → VPS
2. Nginx: create server block for `learn.REDACTED-BRAND-DOMAIN` with Certbot TLS
...
**Exit criteria:**
- `curl https://learn.REDACTED-BRAND-DOMAIN/api/health` returns 200 with all dependencies "ok"
